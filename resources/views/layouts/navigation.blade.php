<aside class="fixed inset-y-0 left-0 z-20 w-64 bg-white border-r border-gray-200 flex flex-col h-screen transition-transform duration-300 ease-in-out transform translate-x-0">
    ...
    <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-1">
        ...
        <a href="{{ route('assets.index') }}" 
           class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors duration-200 group {{ request()->routeIs('assets.*') && !request()->routeIs('assets.print*') && !request()->routeIs('assets.history') ? 'bg-indigo-50 text-indigo-700 font-bold' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
            <svg class="w-5 h-5 {{ request()->routeIs('assets.*') && !request()->routeIs('assets.print*') && !request()->routeIs('assets.history') ? 'text-indigo-600' : 'text-gray-400 group-hover:text-gray-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path></svg>
    ...
    <div class="border-t border-gray-200 p-4 bg-gray-50/50">
        ...
        <form method="POST" action="{{ route('logout') }}" class="form-logout">
            @csrf
            <button type="submit" class="w-full flex items-center justify-center gap-2 px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all cursor-pointer">
